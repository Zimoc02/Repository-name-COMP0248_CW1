# 主训练脚本（跑 train/val、算 Dice/IoU/bbox/class acc，并保存 best_val_dice.pt 和 last.pt）。
import random
import numpy as np
import torch
...
def main():
    ...
    optimizer = torch.optim.Adam(
        [
            ...
            # ---- decoder + seg head (base) ----
            {"params": list(model.up3.parameters()) +
    ...
    best_path = weights_dir / "best_dice_bs10_lrgrp_0.5-1-2_wcls0.5.pt"
    last_path = weights_dir / "last_model_bs10_lrgrp_0.5-1-2_wcls0.5.pt"
    best_cls_path = weights_dir / "best_cls_bs10_lrgrp_0.5-1-2_wcls0.5.pt"
    
    

    train_data_dir = Path("train_data") 
    train_data_dir .mkdir(exist_ok=True)
    log_path = train_data_dir / "trainlog_bs10_lrgrp_0.5-1-2_wcls0.5.csv"
    ...
    for epoch in range(1, epochs + 1):
        ...
        lr_cls = optimizer.param_groups[2]["lr"]
