# 主训练脚本（成功配置版 + tqdm显示 + CSV记录 + 安全命名保存 best/last/best_cls）
import random
import numpy as np
import torch
...
EXP_TAG = f"baseline_320x240_bs{BATCH_SIZE}_lr{BASE_LR:g}_wcls{W_CLS:g}"
...
def main():
    ...
    # data
    train_cfg = DataConfig(csv_path=CSV_PATH, split="train", use_depth=True, keep_original_size=False,
target_size=(320, 240))
    val_cfg = DataConfig(csv_path=CSV_PATH, split="val", use_depth=True, keep_original_size=False,
target_size=(320, 240))
    ...
    if not log_path.exists():
        with open(log_path, "w", newline="", encoding="utf-8") as f:
            ...
            w.writerow([
                ...
                "val_bbox_acc05",
                "val_cls_acc",
                "lr_enc", "lr_seg", "lr_cls",   # 这里保持和你之前一致
            ])
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # 由于这里是单一 lr（Adam(model.parameters())），为了兼容你之前的 CSV 格式：
