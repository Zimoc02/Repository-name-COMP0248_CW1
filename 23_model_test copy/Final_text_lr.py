...
# ✅ 使用你指定的 best_cls 权重
CKPT_PATH = Path(r"C:\Users\zimoc\Desktop\COMP0248_CW1\weights\best_dice_bs10_lrgrp_0.5-1-2_wcls0.5.pt")


TEST_CSV = Path(r"D:\0248_data_check\test_build\COMP0248_Test_data_23\test_index_23_depthpng_for_loader.csv")
OUT_DIR = Path(r"D:\0248_data_check\pred_vis_test\COMP0248_Test_data_23\lr")
...
def run_infer(split="test"):
    ...
    for k, i in pbar:
        ...
        if (k + 1) % 20 == 0 or (k + 1) == len(idxs):
            # 在线统计当前acc（不额外存东西，不影响结果）
            seen = k + 1
            # 这里用 all_pred_rows 最简单
