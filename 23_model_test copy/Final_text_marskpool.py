...
CKPT_PATH = Path(r"C:\Users\zimoc\Desktop\COMP0248_CW1\weights\best_cls_maskpool_bndown4_gtcls1_bs10_lr0.001_wcls0.5_20260305_060640.pt")


TEST_CSV = Path(r"D:\0248_data_check\test_build\COMP0248_Test_data_23\test_index_23_depthpng_for_loader.csv")
OUT_DIR = Path(r"D:\0248_data_check\pred_vis_test\COMP0248_Test_data_23\maskpool")
...
def run_infer(split="test"):
    ...
    # Model
    model = UNetMultiTask_MaskPool(in_channels=4, num_classes=10, base_ch=32).to(device)
    ...
    for k, i in pbar:
        ...
        with torch.no_grad():
            ...
            # ✅ 兼容不同 forward 输出格式（tuple/list 或 dict）
        ...
        if (k + 1) % 20 == 0 or (k + 1) == len(idxs):
            seen = k + 1
            correct_now = sum(1 for r in all_pred_rows if r["gt"] == r["pred"])
            pbar.set_postfix(acc=f"{correct_now/seen:.3f}")
